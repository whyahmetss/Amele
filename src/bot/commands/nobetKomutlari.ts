...
export function nobetKomutlariniKaydet(bot: TelegramBot): void {
  ...
  bot.onText(/^\/nobet haftal[iı]k/i, async (mesaj) => {
    try {
      const list = await nobetService.haftalikGetir();
      if (!list.length) {
  ...
  bot.onText(/^\/nobet ata (.+)$/i, async (mesaj, eslesme) => {
    ...
    const parseTarih = (s: string): string | null => {
      const bugun = new Date();
      if (/^(\d{4})-(\d{2})-(\d{2})$/.test(s)) return s;
      if (s === 'bugün' || s === 'bugun') return bugun.toISOString().split('T')[0];
      if (s === 'yarın' || s === 'yarin') {
    ...
    const kayit = await nobetService.ekle(uid, ad, tarih);
    if (kayit) {
